...
df = df.dropna(subset=['OR'], how='all') # clean the SNP if it doesn't have OR score
...
with open(fileName+"_"+refName+"_PRS.txt",'w+') as f:
    with open(fileName+"_"+refName+".txt",'a+') as g:
        ...
        for line in VCF.readlines():
            ...
            filter = (df["chr_name"]==CHROM)            #filter will let vcf only find it's chromosome number in reference.
            for index ,row in df[filter].iterrows():    #eg. if chr=1 in vcf , it will only find chr=1 in reference,nor other chr.
                if row["chr_position"]==POS and row["effect_allele"] ==REF and row["other_allele"] in ALT:# check whether position、allele are the same
                    for i in df.loc[index].tolist(): # write .txt
                        g.write(str(i)+"\t")
                    g.write("\n")
                    print("find one!")
                    Or = math.log(float(df.loc[index]["OR"]))  #natural log of OR
